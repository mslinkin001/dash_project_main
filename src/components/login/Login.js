  ...
  const LoginHandler = (e) => {
    ...
    if (!enteredUsername || !enteredPassword) {
      ...
      if (enteredUsername === "headmaster") {
        fetchUrl =
          "/admin_table" +
          `?where={ "position": "مدیر","password": "${enteredPassword}"}`;
        enteredPosition = "مدیر";
      } else if (enteredUsername === "teacher") {
        fetchUrl =
          "/teacher_table" +
          `?where={ "position": "معلم","password": "${enteredPassword}"}`;
        enteredPosition = "معلم";
      } else if (enteredUsername === "student") {
        fetchUrl =
          "/student_table" +
          `?where={ "position": "دانش آموز","password": "${enteredPassword}"}`;
        enteredPosition = "دانش آموز";
      }

      props.loginHandler(fetchUrl, "GET", enteredPosition);
    }
  };
